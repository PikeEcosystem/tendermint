...
import (
	...
	"github.com/PikeEcosystem/tendermint/p2p/conn"
	p2pmock "github.com/PikeEcosystem/tendermint/p2p/mock"
	p2pmocks "github.com/PikeEcosystem/tendermint/p2p/mocks"
...
func TestNewtendermintNode(t *testing.T) {
	config := cfg.ResetTestRootWithChainID("TestNewtendermintNode", "new_tendermint_node")
	defer os.RemoveAll(config.RootDir)
	require.Equal(t, "", config.PrivValidatorListenAddr)
	node, err := NewtendermintNode(config, log.TestingLogger())
...
func TestNewtendermintNode_WithoutNodeKey(t *testing.T) {
	config := cfg.ResetTestRootWithChainID("TestNewtendermintNode", "new_tendermint_node_wo_node_key")
	defer os.RemoveAll(config.RootDir)
	_ = os.Remove(config.NodeKeyFile())
	_, err := NewtendermintNode(config, log.TestingLogger())
	require.Error(t, err)
}
