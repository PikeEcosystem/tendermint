...
func (app *Application) CheckTxSync(req types.RequestCheckTx) tmabci.ResponseCheckTx {
	return app.checkTx(req)
}

func (app *Application) CheckTxAsync(req types.RequestCheckTx, callback tmabci.CheckTxCallback) {
	callback(app.checkTx(req))
}

func (app *Application) checkTx(req types.RequestCheckTx) tmabci.ResponseCheckTx {
	if app.serial {
		if len(req.Tx) > 8 {
