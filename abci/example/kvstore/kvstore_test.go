...
func testKVStore(t *testing.T, app tmabci.Application, tx []byte, key, value string) {
...
func TestPersistentKVStoreInfo(t *testing.T) {
	...
	kvstore.BeginBlock(tmabci.RequestBeginBlock{Hash: hash, Header: header})
...
func existInPersistStore(t *testing.T, kvstore tmabci.Application, v types.ValidatorUpdate) {
	// success
...
func valsEqual(t *testing.T, vals1, vals2 []types.ValidatorUpdate) {
	...
	sort.Sort(tmabci.ValidatorUpdates(vals1))
	sort.Sort(tmabci.ValidatorUpdates(vals2))
...
func makeSocketClientServer(app tmabci.Application, name string) (abcicli.Client, service.Service, error) {
	// Start the listener
...
func makeGRPCClientServer(app tmabci.Application, name string) (abcicli.Client, service.Service, error) {
	// Start the listener
	socket := fmt.Sprintf("unix://%s.sock", name)
	logger := log.TestingLogger()

	gapp := tmabci.NewGRPCApplication(app)
