...
var _ tmabci.Application = (*PersistentKVStoreApplication)(nil)
...
func (app *PersistentKVStoreApplication) CheckTxSync(req types.RequestCheckTx) tmabci.ResponseCheckTx {
	return app.app.CheckTxSync(req)
}

func (app *PersistentKVStoreApplication) CheckTxAsync(req types.RequestCheckTx, callback tmabci.CheckTxCallback) {
	app.app.CheckTxAsync(req, callback)
}

func (app *PersistentKVStoreApplication) BeginRecheckTx(req tmabci.RequestBeginRecheckTx) tmabci.ResponseBeginRecheckTx {
	return app.app.BeginRecheckTx(req)
}

func (app *PersistentKVStoreApplication) EndRecheckTx(req tmabci.RequestEndRecheckTx) tmabci.ResponseEndRecheckTx {
...
func (app *PersistentKVStoreApplication) BeginBlock(req tmabci.RequestBeginBlock) types.ResponseBeginBlock {
...
func (app *PersistentKVStoreApplication) Validators() (validators []types.ValidatorUpdate) {
	...
	for ; itr.Valid(); itr.Next() {
		if isValidatorTx(itr.Key()) {
			...
			err := tmabci.ReadMessage(bytes.NewBuffer(itr.Value()), validator)
			if err != nil {
				panic(err)
			}
...
// format is "val:pubkey!power"
// pubkey is a base64-encoded proto.tendermint.crypto.PublicKey bytes
...
func (app *PersistentKVStoreApplication) execValidatorTx(tx []byte) types.ResponseDeliverTx {
	...
	// update
	return app.updateValidator(tmabci.NewValidatorUpdate(pubkey, power))
}
