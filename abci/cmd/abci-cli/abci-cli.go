...
import (
	...
	"github.com/PikeEcosystem/tendermint/abci/server"
	servertest "github.com/PikeEcosystem/tendermint/abci/tests/server"
	tmabci "github.com/PikeEcosystem/tendermint/abci/types"
	"github.com/PikeEcosystem/tendermint/abci/version"
...
func cmdPersistKVStoreMakeValSetChangeTx(cmd *cobra.Command, args []string) error {
	...
	c.SetRoot(filepath.Join(userHome, config.DefaulttendermintDir))
	...
	}
	{
		...
		err = tmabci.ReadMessage(bytes.NewReader(res.Value), &validatorUpdate)
		if err != nil {
			panic(err)
		}
