...
func (app *Application) CheckTx(req abci.RequestCheckTx) tmabci.ResponseCheckTx {
...
func (app *Application) validatorUpdates(height uint64) (tmabci.ValidatorUpdates, error) {
	updates := app.cfg.ValidatorUpdates[fmt.Sprintf("%v", height)]
	if len(updates) == 0 {
		return nil, nil
	}

	valUpdates := tmabci.ValidatorUpdates{}
	for keyString, power := range updates {
		...
		valUpdates = append(valUpdates, tmabci.NewValidatorUpdate(pubKey, int64(power)))
	}
