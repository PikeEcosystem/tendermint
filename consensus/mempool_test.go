...
func TestMempoolRmBadTx(t *testing.T) {
	...
	resBeginRecheckTx := app.BeginRecheckTx(tmabci.RequestBeginRecheckTx{})
	assert.Equal(t, code.CodeTypeOK, resBeginRecheckTx.Code)

	// There is no tx to recheck

	resEndRecheckTx := app.EndRecheckTx(tmabci.RequestEndRecheckTx{})
...
func (app *CounterApplication) CheckTxSync(req abci.RequestCheckTx) tmabci.ResponseCheckTx {
	return app.checkTx(req)
}

func (app *CounterApplication) CheckTxAsync(req abci.RequestCheckTx, callback tmabci.CheckTxCallback) {
	callback(app.checkTx(req))
}

func (app *CounterApplication) checkTx(req abci.RequestCheckTx) tmabci.ResponseCheckTx {
	txValue := txAsUint64(req.Tx)
	...
	app.mempoolTxCount++
	return tmabci.ResponseCheckTx{Code: code.CodeTypeOK}
}

func (app *CounterApplication) BeginRecheckTx(tmabci.RequestBeginRecheckTx) tmabci.ResponseBeginRecheckTx {
	app.mempoolTxCount = app.txCount
	return tmabci.ResponseBeginRecheckTx{Code: code.CodeTypeOK}
}
