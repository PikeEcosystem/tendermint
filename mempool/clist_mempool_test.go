...
func TestMempoolFilters(t *testing.T) {
	...
	for tcIndex, tt := range tests {
		err := mempool.Update(newTestBlock(1, emptyTxArr),
			abciResponses(len(emptyTxArr), tmabci.CodeTypeOK), tt.preFilter, nil)
...
func TestMempoolUpdate(t *testing.T) {
	...
	// 1. Adds valid txs to the cache
	{
		err := mempool.Update(newTestBlock(1, []types.Tx{[]byte{0x01}}),
			abciResponses(1, tmabci.CodeTypeOK), nil, nil)
		require.NoError(t, err)
	...
	// 2. Removes valid txs from the mempool
	{
		...
		require.NoError(t, err)
		err = mempool.Update(newTestBlock(1, []types.Tx{[]byte{0x02}}), abciResponses(1, tmabci.CodeTypeOK), nil, nil)
		require.NoError(t, err)
...
func TestMempool_KeepInvalidTxsInCache(t *testing.T) {
	...
	// 1. An invalid transaction must remain in the cache after Update
	{
		...
		err = mempool.Update(newTestBlock(1, []types.Tx{a, b}),
			[]*abci.ResponseDeliverTx{{Code: tmabci.CodeTypeOK}, {Code: 2}}, nil, nil)
		require.NoError(t, err)
...
func TestTxsAvailable(t *testing.T) {
	...
	if err := mempool.Update(newTestBlock(1, committedTxs),
		abciResponses(len(committedTxs), tmabci.CodeTypeOK), nil, nil); err != nil {
		t.Error(err)
	}
	ensureFire(t, mempool.TxsAvailable(), timeoutMS)
	ensureNoFire(t, mempool.TxsAvailable(), timeoutMS)
...
func TestSerialReap(t *testing.T) {
	...
	updateRange := func(start, end int) {
		...
		if err := mempool.Update(newTestBlock(0, txs),
			abciResponses(len(txs), tmabci.CodeTypeOK), nil, nil); err != nil {
			t.Error(err)
		}
	}
...
func TestMempoolTxsBytes(t *testing.T) {
	...
	err = mempool.Update(newTestBlock(1, []types.Tx{}), abciResponses(0, tmabci.CodeTypeOK), nil, nil)
	require.NoError(t, err)
	assert.EqualValues(t, 0, mempool.TxsBytes())
...
func TestTxMempoolPostCheckError(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			mempool.postCheck = func(_ types.Tx, _ *tmabci.ResponseCheckTx) error {
				return testCase.err
			}
			...
			m := sync.Mutex{}
			m.Lock()
			mempool.proxyAppConn.CheckTxAsync(req, func(r *tmabci.Response) {
				res = r
				m.Unlock()
			})

			checkTxRes, ok := res.Value.(*tmabci.Response_CheckTx)
			require.True(t, ok)
			expectedErrString := ""
			if testCase.err != nil {
